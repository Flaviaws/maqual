...
* O terceiro argumento da função é *lvl*, um objeto do tipo vetor numérico.
  ...
  *lvl* nos dará o valor do nível (elemento da matriz) para qual a função será 
  ...
  Iremos carregar um dataframe presente no conjunto de dados do pacote, pelo 
...
* O quarto argumento da função é *q*, um objeto do tipo vetor numérico.
  ...
  *q* nos dará o Valor de peso por qual o nível especificado será ponderado.
  ...
  O valor de *q* está intimamente recionado ao valor de *lvl*. Portanto, iremos 
  seleciona o valor de *q* para o mesmo habitat, o de 'Floresta madura'.
```{r}
q_f <- info$q_peso[1]
q_f
```
* O último argumento da função é *na.rm*, um objeto do tipo vetor de caracteres.
  ...
  *na.rm* é um argumento lógico (*TRUE* ou *FALSE*) que nos dirá se o 'NA's
...
```{r}
qual_f <- quality(m, fm, lvl = lvl_f, q = lvl_f, na.rm)
qual_f
```
Observe que o objeto 'info' disponível no pacote contém vários valores para 'lvl'
e 'q'. 

Vamos observar como o resultado com outros valores nestes parâmetros,
resultando na qualidade da matriz para outros habitats.
...
qual_p <- quality(m, fm, lvl = lvl_p, q = lvl_p, na.rm)
qual_p
```
...
qual_u <- quality(m, fm, lvl = lvl_u, q = lvl_u, na.rm)
qual_u
```
